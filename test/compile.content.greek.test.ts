...
import { markit, markitWithContent, p, pt } from "./utils/factories.ts";

describe("Greek mode", () => {
  it("transliterates basic Latin characters to Greek", () => {
    const { document, errors } = compile(
      markitWithContent("{#1}", "{{logos}}"),
    );
