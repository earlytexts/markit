...
import { guardMarkitDocument } from "./lib/documentGuard.ts";
import renderHtml from "./lib/renderHtml.ts";
import injectPreviewAssets from "./lib/previewAssets.ts";

export default (context: ExtensionContext): (() => Promise<void>) => {
  ...
  return async () => {
    ...
    const guardMessage = guardMarkitDocument(document);
    if (guardMessage) {
...
const updatePreview = (
  ...
  const { document } = compileWithPositions(content);
  ...
  previewPanel.webview.html = injectPreviewAssets(
    html,
    cssUri.toString(),
    jsUri.toString(),
  );
